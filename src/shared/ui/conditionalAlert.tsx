...
const ConditionalAlert = memo(
  ({ status, errors, setStatus }: ConditionalAlertProps): React.JSX.Element => {
    ...
    return (
      <div className="flex flex-col justify-center items-center gap-4 mb-3 text-[14px]">
        <Alert
          color="danger"
          isVisible={isVisible}
          variant="flat"
          classNames={{
            closeButton: "flex items-center justify-center translate-none"
          }}
          onClose={handleClose}
        >
          {typeof axiosError !== "string" && axiosError !== undefined ? (
            ...
            <span>{normalizeMessage(axiosError)}</span>
          )}
        </Alert>
      </div>
    );
