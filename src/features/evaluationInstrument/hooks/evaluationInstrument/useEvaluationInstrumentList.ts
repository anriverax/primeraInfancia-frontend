...
import { IEvaluationInstrumentTable, EvaluationInstrumentListResult } from "../../evaluationInstrumentType";
...
const useEvaluationInstrumentsList = (): EvaluationInstrumentListResult => {
  ...
  useEffect(() => {
    ...
    const fetchData = async (): Promise<void> => {
      try {
        const res: AxiosResponse<FetchResponse<IEvaluationInstrumentTable[]>> = await useRequest.get("/evaluationInstrument");
