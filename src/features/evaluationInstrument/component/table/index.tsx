...
import { IEvaluationInstrumentColumnKey, IEvaluationInstrumentTable, EvaluationInstrumentTableProps } from "../../evaluationInstrumentType";
import { tableClassNames } from "@/shared/constants";
import { confirmDelete } from "@/shared/utils/funtions";

const EvaluationInstrumentTable = ({ evaluationInstrumentsList }: EvaluationInstrumentTableProps): React.JSX.Element => {
    ...
    <div className="space-y-4">
      ...
      <Table classNames={tableClassNames} aria-label="Tabla para mostrar las zonas registradas">
        <TableHeader columns={evaluationInstrumentColumns}>
          {(evaluationInstrumentCol) => <TableColumn key={evaluationInstrumentCol.key}>{evaluationInstrumentCol.label}</TableColumn>}
        </TableHeader>
        <TableBody isLoading={evaluationInstrumentsList.length === 0} items={evaluationInstrumentsList}>
          {(evaluationInstrumentItem: IEvaluationInstrumentTable) => (
            <TableRow key={evaluationInstrumentItem.id}>
              {(evaluationInstrumentKey) => <TableCell>{renderEvaluationInstrumentCell(evaluationInstrumentItem, evaluationInstrumentKey as IEvaluationInstrumentColumnKey)}</TableCell>}
            </TableRow>
          )}
        </TableBody>
      </Table>
