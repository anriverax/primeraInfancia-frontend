...
import { ITrainingEvaluationColumnKey, ITrainingEvaluationTable, TrainingEvaluationTableProps } from "../../trainingEvaluationType";
import { tableClassNames } from "@/shared/constants";
import { confirmDelete } from "@/shared/utils/funtions";

const TrainingEvaluationTable = ({ trainingEvaluationsList }: TrainingEvaluationTableProps): React.JSX.Element => {
    ...
    <div className="space-y-4">
      ...
      <Table classNames={tableClassNames} aria-label="Tabla para mostrar las zonas registradas">
        <TableHeader columns={ trainingEvaluationColumns }>
          { (trainingEvaluationCol) => <TableColumn key={ trainingEvaluationCol.key }>{ trainingEvaluationCol.label }</TableColumn> }
        </TableHeader>
        <TableBody isLoading={ trainingEvaluationsList.length === 0} items={ trainingEvaluationsList }>
          { (trainingEvaluationItem: ITrainingEvaluationTable) => (
            <TableRow key={ trainingEvaluationItem.id }>
              { (trainingEvaluationKey) => <TableCell>{ renderTrainingEvaluationCell(trainingEvaluationItem, trainingEvaluationKey as ITrainingEvaluationColumnKey) }</TableCell> }
            </TableRow>
          )}
        </TableBody>
      </Table>
