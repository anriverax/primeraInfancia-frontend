...
import { IModuleReportColumnKey, IModuleReportTable, ModuleReportTableProps } from "../../moduleReportType";
    ...
    <div className="space-y-4">
      ...
      <Table classNames={tableClassNames} aria-label="Tabla para mostrar las zonas registradas">
        <TableHeader columns={ moduleReportColumns }>
          { (moduleReportCol) => <TableColumn key={ moduleReportCol.key }>{ moduleReportCol.label }</TableColumn> }
        </TableHeader>
        <TableBody isLoading={ moduleReportsList.length === 0} items={ moduleReportsList }>
          { (moduleReportItem: IModuleReportTable) => (
            <TableRow key={ moduleReportItem.id }>
              { (moduleReportKey) => <TableCell>{ renderModuleReportCell(moduleReportItem, moduleReportKey as IModuleReportColumnKey) }</TableCell> }
            </TableRow>
          )}
        </TableBody>
      </Table>
