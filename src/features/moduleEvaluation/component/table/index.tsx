...
}: ModuleEvaluationTableProps): React.JSX.Element => {
    ...
    <div className="space-y-4">
      ...
      <Table classNames={tableClassNames} aria-label="Tabla para mostrar las zonas registradas">
        <TableHeader columns={moduleEvaluationColumns}>
          {(moduleEvaluationCol) => (
            <TableColumn key={moduleEvaluationCol.key}>{moduleEvaluationCol.label}</TableColumn>
          )}
        </TableHeader>
        <TableBody isLoading={moduleEvaluationsList.length === 0} items={moduleEvaluationsList}>
          {(moduleEvaluationItem: IModuleEvaluationTable) => (
            <TableRow key={moduleEvaluationItem.id}>
              {(moduleEvaluationKey) => (
                <TableCell>
                  {renderModuleEvaluationCell(
                    moduleEvaluationItem,
                    moduleEvaluationKey as IModuleEvaluationColumnKey
                  )}
                </TableCell>
              )}
            </TableRow>
