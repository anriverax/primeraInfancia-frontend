...
const getCSP = (): string => {
  const cspDirectives = [
    ...
    `script-src 'self' ${isDev ? "'unsafe-inline' 'unsafe-eval'" : ""} https://cdn.jsdelivr.net`,
    ...
    `connect-src 'self' ${isDev ? "http://localhost:3001 ws: wss: http://localhost:* 127.0.0.1:*" : ""} https:`,
    "form-action 'self'",
